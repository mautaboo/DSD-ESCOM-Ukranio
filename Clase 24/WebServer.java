...
private void handleTaskRequest(HttpExchange exchange) throws IOException { //Manejador del endpoint task
    ...
    byte[] responseBytes = deserializacionwb(requestBytes); //Pasa por este método 
    long finishTime = System.nanoTime();
    byte[] retunnmsString = String.format("Nanosegundos: %d", finishTime - startTime).getBytes();

    if (isDebugMode) {
        String debugMessage = String.format("Nanosegundos: %d", finishTime - startTime); // Devolvemos la cantidad de timepo 
        exchange.getResponseHeaders().put("X-Debug-Info", Arrays.asList(debugMessage)); //Lo introducimos en el header X-Debug-info
    }
    sendResponse(retunnmsString, exchange);
}

private byte[] deserializacionwb(byte[] requestBytes) {
    ...
    return String.format("Operacion realizada con exito\n").getBytes();
}
